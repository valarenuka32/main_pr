...
const registerAdmin = async (req, res) => {
  try {
    ...
    if (body.password === body.confirm_password) {
      ...
      res
        .status(400)
        .json({ message: "pasword does not mathc", success: false });
    }
  } catch (error) {
...
//     var admin_data = await adminServices.find({
//       username: { $regex: ".*" + search + "." },
//     });
//     if (admin_data.length > 0) {
//       res
//         .status(200)
//         .json({ success: true, message: "admin data", data: admin_data });
//     } else {
